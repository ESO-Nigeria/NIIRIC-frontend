...
const EmblaCarousel: React.FC<PropType> = ({
  ...
  const mergedOptions = { ...options, slidesToScroll: 1, align: 'start', dragFree: false, containScroll: 'trimSnaps', ...(slidesToShow ? { slidesToScroll: slidesToShow } : {}) }
  const [emblaRef, emblaApi] = useEmblaCarousel(
    { ...mergedOptions, slidesToScroll: 1, align: 'start', dragFree: false, containScroll: 'trimSnaps', ...(slidesToShow ? { slidesToScroll: slidesToShow } : {}) },
    [Autoplay()]
  )
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      else if (window.innerWidth < 1024) setVisibleSlides(Math.min(2, slides.length))
