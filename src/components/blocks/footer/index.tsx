...
import { useSubscribeNewsletterMutation } from "@/store/features/general/actions";
			...
			{withSubscribe && (
				<section className="relative border-t-1 border-primary-green/10 bg-white py-22 px-6 lg:px-8">
					...
					<form onSubmit={handleSubmit} className="flex flex-col sm:flex-row items-center w-full">
						<div className="relative bg-primary-brown/25 border rounded-lg h-14 flex items-center w-full sm:flex-1">
						<input
							type="email"
							placeholder="Enter your email"
							value={email}
							onChange={(e) => setEmail(e.target.value)}
							className="border-none outline-none rounded-lg p-2 flex-1 h-full w-full"
							required
						/>
						<button
							type="submit"
							className="absolute right-2 top-1/2 -translate-y-1/2 bg-primary-green text-white rounded-lg px-4 py-2 flex items-center justify-center transition hover:bg-primary-green/90 disabled:opacity-50 disabled:cursor-not-allowed"
							disabled={isLoading}
						>
							Submit Now
				...
				<section className="bg-white py-20 px-6 lg:px-8">
					<div className="container  mx-auto  relative max-w-7xl">
						<div className="space-y-4">
							<h2 className="text-3xl font-bold text-primary-green  text-center leading-12 sm:text-4xl">
								<span className="block">Our Partners and Sponsors</span>
							</h2>
						</div>
