...
const slides = [
  {
    id: 1,
    imgSrc: "/assets/images/what_we_do_banner.png",
...
export default function HeroCarouselWithIndicatorsAndAutoplay() {
  return (
      ...
      <Carousel opts={{ align: "start", loop: true, autoplay: true, interval: 5000 }} className="w-full">
        <CarouselContent>
          {slides.map((slide) => (
        ...
        <div className="absolute bottom-15 left-1/2 transform z-30   -translate-x-1/2 flex gap-2">
          {/* {slides.map((_, index) => ( */}
            <CarouselDots selectedstyle='bg-[#D1AE6F] !w-4 !h-2 rounded-full' dotstyle="w-2.5 h-2 rounded-full " className=" rounded-full" />
          {/* ))} */}
        </div>
      </Carousel>
