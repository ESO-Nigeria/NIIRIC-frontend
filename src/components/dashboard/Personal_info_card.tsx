...
import { Mail, Phone, Linkedin, Link as LinkIcon, Pencil, ContactRound } from "lucide-react";
...
export function PersonalInfoCard({ data, onEdit, isLoading }: PersonalInfoCardProps) {
  ...
  if (!data) {
    return (
      <Card>
        <CardContent className="p-6 text-gray-500">No personal info available.</CardContent>
      </Card>
    );
  }
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 space-y-2">
            <h2 className="text-xl text-[#3F434A] medium capitalize">
              {title ? `${title} ${first_name} ${last_name}` : `${first_name} ${last_name}`}
            </h2>
            {state && <p className="text-sm text-[#475467] capitalize">{state}, Nigeria</p>}
         ...
         <div className="flex space-y-2 flex-col">
           <h5 className="text-xl text-[#3F434A] medium  ">Bio</h5>
                {bio && <p className="text-sm text-[#475467] leading-relaxed">{bio}</p>}
            ...
            <div className="ml-auto" >
              ...
              onClick={onEdit}
              className={clsx(buttonVariants({variant: "link", size: 'sm'}), "ml-auto text-primary-green font-normal underline text-base ")}
            >
