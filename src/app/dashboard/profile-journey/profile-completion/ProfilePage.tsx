...
import { UploadCloud, Mail, Contact2 } from "lucide-react";
import { useEffect, useState } from "react";
import Image from "next/image";
import { useEditUserProfileMutation, useGetUserProfileQuery, useUpdateUserProfileMutation } from "@/store/features/auth/actions";
...
const formSchema = z.object({
  ...
  orcid: z
    .string()
    .optional()
    .refine((val) => !val || /^https?:\/\/orcid\.org\/\d{4}-\d{4}-\d{4}-\d{4}$/.test(val), {
      message: "Enter a valid ORCID URL (https://orcid.org/0000-0000-0000-0000)",
    }),
...
export default function ProfileCompletionForm() {
  ...
  const  onSubmit = async (values: FormValues) => {
    ...
    try {
      const {data, error} = await (id ? editUserProfile(data_to_send) : updateUserProfile(data_to_send));
      if(data){
        ...
        <Form {...form}>
          <form className="space-y-6">
                    ...
                    <div className="rounded-md border border-gray-200  p-6 flex  items-start gap-4 ">
                      ...
                      >
                        ...
                        <p className="text-sm text-[#3F434A] ">
                          Upload a professional photo. This will be visible to other
                          researchers
                        </p>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-base text-[#242424] font-normal ">Title</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="First Name" {...field} className="h-11 focus-visible:ring-0"/>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Last Name" {...field} className="h-11 focus-visible:ring-0" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-base text-[#242424] font-normal ">Email Address</FormLabel>
                    <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-base text-[#242424] font-normal ">Phone Number</FormLabel>
                    <FormControl>
                      <Input placeholder="Enter Phone Number" {...field} className="h-11 focus-visible:ring-0"/>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="linkedin.com/profile-name" {...field} className="h-11 focus-visible:ring-0" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-base text-[#242424] font-normal ">ORCID</FormLabel>
                    <FormControl>
                      <Input placeholder="Enter ORCID" {...field} className="h-11 focus-visible:ring-0" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-base text-[#242424] font-normal ">Bio</FormLabel>
                  <FormControl>
                    <Textarea
            ...
            <div className="flex justify-end gap-3">
              <Button disabled className="w-[215px]" variant="outline" type="button">
                Cancel
              </Button>
              <Button  type="button"
                onClick={handleOpenModal}
              variant={"primary-green"}  className="w-[215px] text-white">
                Save Changes
              </Button>
            </div>
          </form>
