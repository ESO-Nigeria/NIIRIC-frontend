...
export default function QualificationForm() {
  ...
  useEffect(() => {
    if (
      edit &&
      userQualifications &&
      Array.isArray(userQualifications?.results)
    ) {
      const mappedQualifications = userQualifications?.results.map(
        (qual: any) => ({
          id: qual.id,
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      const newQualifications = values.qualifications.filter(
        (qual) => !qual.id
      );

      if (newQualifications.length > 0) {
        // Create new qualifications (without id field)
        const qualificationsToCreate = newQualifications.map(
          ({ id, ...rest }) => rest
        );
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                          ...
                          <FormControl>
                            ...
                            >
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="english_literature">
                                  English Literature
                                </SelectItem>
            ...
            <div className="flex justify-end gap-4">
              ...
              <Button
                ...
                className="w-[215px]"
                disabled={
                  isSubmitting || (locked.length > 0 && !locked.every(Boolean))
                }
              >
                {isSubmitting
                  ? "Saving..."
