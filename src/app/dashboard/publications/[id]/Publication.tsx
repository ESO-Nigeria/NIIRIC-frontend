...
import {
  ...
  useRecordPublicationViewMutation,
...
const useFollowUser = (authorId?: string | undefined) => {
  ...
  const handleFollowToggle = async (currentlyFollowing: boolean) => {
    ...
    try {
      if (currentlyFollowing) {
       const response = await unfollowUser(authorId).unwrap();
        setIsFollowing(false);
        toast.success(response?.message || "Unfollowed successfully");
      } else {
     const response =  await followUser(authorId).unwrap();
        setIsFollowing(true);
        console.log(response, 'response from follow');
        toast.success(response?.message || "Following successfully");
      }
    } catch (err) {
...
const useComments = (id: string | string[] | undefined) => {
  ...
  const { data: commentsData, refetch } = useGetCommentsQuery(commentFilters);
  ...
  const handleCommentLikeToggle = async (comment: any, action: string) => {
    try {
      await likeOrUnlikeComment({ comment: comment.id, flag: action }).unwrap();
      refetch();
    } catch (err) {
      console.error("Failed to toggle like:", err);
    }
    ...
    const recordView = async () => {
      ...
      if (!hasViewed) {
        try {
          ...
          console.log("Publication view recorded");
        } catch (err) {
    ...
    return authors.map((author, idx) => (
      <div key={idx} className="flex gap-3 items-center">
        ...
        <Link
          ...
          className="font-normal text-base text-[#3F434A]"
        >
